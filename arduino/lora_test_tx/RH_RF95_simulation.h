...
public:
  ...
  virtual bool recv(uint8_t* buf, uint8_t* len) {
    ...
    strncpy((char*)buf, str, (size_t)(*len));
    *len = strlen(str);
    return true;
  }
  ...
  uint8_t getSpreadingFactor() { return m_sf; }
  ...
  long getSignalBandwidth() { return m_sbw; }
  ...
  virtual uint16_t rxBad()  { return _rxBad++;  }
  virtual void     resetRxBad()	{ _rxBad = 0; }

  virtual uint16_t rxGood() { return _rxGood++; }
  virtual void     resetRxGood()	{ _rxGood = 0; }

  virtual uint16_t txGood() { return _txGood; }
  virtual void     resetTxGood()	{ _txGood = 0; }

  // Tx
  virtual bool waitPacketSent()                                                 { return true; }
  virtual bool waitAvailableTimeout(uint16_t timeout, uint16_t polldelay = 0)   { return true; }
